...
import { Button, H1, Paragraph, ScrollView, Spinner, Text, useTheme, YStack } from "tamagui";

import { Container, Main, Subtitle, Title } from "~/tamagui.config";
import { ImageBackground } from "react-native";
import Animated from "react-native-reanimated";
import { MovieList } from "~/app/screens/(drawer)/(tabs)/home/movies/movie-list/movie-list";
...
export default function DetailsPage() {
  ...
  const onChooseFavorite = (): void => {
    dispatch(addToFavorite({ item: movieItem }));
  };
  const toggleFavorite = () => {
    console.log("toggleFavorite", movieItem);
    isFavorite ?
      ...
      <ScrollView>
        <ImageBackground
          ...
          <Animated.Image
            ...
            sharedTransitionTag={`${movieItem?.media_type === 'movie' ? 'movie' : 'tv'}-${id}`}
          />
        </ImageBackground>
        ...
        <YStack
          ...
          }
          {
            ...
            <Paragraph fontSize={16}>Average number of votes: {movieItem?.vote_average.toFixed(1)}</Paragraph>
          }
